...
function configKey(config) {
  return `${_nullishCoalesce(config.baseUrl, () => ( ""))}|${config.email}|${_nullishCoalesce(config.userAgent, () => ( ""))}`;
}
function createTexas811Server(config) {
  ...
  const baseUrl = _nullishCoalesce(config.baseUrl, () => ( "https://txgc.texas811.org"));
  const timeoutMs = _nullishCoalesce(config.timeoutMs, () => ( 3e4));
  const userAgent = _nullishCoalesce(config.userAgent, () => ( "Texas811Client/1.0"));
  ...
  function normalizeSessionError(error, context) {
    if (_axios2.default.isAxiosError(error)) {
      ...
      const status = _optionalChain([error, 'access', _ => _.response, 'optionalAccess', _2 => _2.status]);
      if (status) {
        return new Error(
  ...
  function buildTicketXml(ticketData) {
    ...
    return `<ticket xmlns="http://schemas.progressivepartnering.com/geocall/v3/1/types/app">
        <suspendedCount>${escapeXml(String(_nullishCoalesce(ticketData.suspendedCount, () => ( 0))))}</suspendedCount>
        ...
        <hoursNotice>${escapeXml(String(_nullishCoalesce(ticketData.hoursNotice, () => ( 48))))}</hoursNotice>
        <compliance>${escapeXml(String(_nullishCoalesce(ticketData.compliance, () => ( 0))))}</compliance>
        ...
        <workSite>
        <callerSuppliedPoints>${escapeXml(String(_nullishCoalesce(ws.callerSuppliedPoints, () => ( "undefined"))))}</callerSuppliedPoints>
  ...
  function parseUpdateResponse(xml) {
    ...
    return {
      newTicketNumber: _optionalChain([newTicketMatch, 'optionalAccess', _3 => _3[1]]) || _optionalChain([ticketNumberMatch, 'optionalAccess', _4 => _4[1]]) || null,
      oldTicketNumber: _optionalChain([oldTicketMatch, 'optionalAccess', _5 => _5[1]]) || null,
  ...
  function parseCreateResponse(xml) {
    ...
    return {
      ticketId: _optionalChain([ticketIdMatch, 'optionalAccess', _6 => _6[1]]) || null,
      ticketNumber: _optionalChain([ticketNumberMatch, 'optionalAccess', _7 => _7[1]]) || null,
      rawResponse: xml
    };
  }
  ...
  function parseBeginResponse(xml) {
    const startedOn = _nullishCoalesce(_optionalChain([xml, 'access', _8 => _8.match, 'call', _9 => _9(/<startedOn>([^<]+)<\/startedOn>/i), 'optionalAccess', _10 => _10[1]]), () => ( ""));
    const saversion = _nullishCoalesce(_optionalChain([xml, 'access', _11 => _11.match, 'call', _12 => _12(/<sa>([^<]+)<\/sa>/i), 'optionalAccess', _13 => _13[1]]), () => ( ""));
    const baseversion = _nullishCoalesce(_optionalChain([xml, 'access', _14 => _14.match, 'call', _15 => _15(/<base>([^<]+)<\/base>/i), 'optionalAccess', _16 => _16[1]]), () => ( ""));
    const workOnMatch = xml.match(
      /<calculatedDate>\s*<key>normal-WorkOn<\/key>\s*<value>([^<]+)<\/value>\s*<\/calculatedDate>/i
    );
    const workOn = _nullishCoalesce(_optionalChain([workOnMatch, 'optionalAccess', _17 => _17[1]]), () => ( ""));
  ...
  async function createTicketWithSession(token, ticketData, temporaryTicketId) {
    const jar = new (0, _toughcookie.CookieJar)();
    const session = _axioscookiejarsupport.wrapper.call(void 0, 
    ...
    if (workonData) {
      const wOn = _nullishCoalesce(workonData["normal-WorkOn"], () => ( workonData.workOn));
      if (wOn) ticketData.workOn = wOn;
      const hNotice = _nullishCoalesce(workonData["normal-HoursNotice"], () => ( workonData.hoursNotice));
      if (hNotice != null) ticketData.hoursNotice = Number(hNotice);
      const comp = _nullishCoalesce(workonData["normal-Compliance"], () => ( workonData.compliance));
    ...
    if (submitResp.status < 200 || submitResp.status >= 300) {
      const detail = String(_nullishCoalesce(submitResp.data, () => ( ""))).trim().slice(0, 500);
      throw new Error(
...
exports.UPDATE_ONLY_FLAG = UPDATE_ONLY_FLAG; exports.CANCEL_FLAG = CANCEL_FLAG; exports.NO_RESPONSE_PREFIX = NO_RESPONSE_PREFIX; exports.NO_RESPONSE_SUFFIX = NO_RESPONSE_SUFFIX; exports.createTexas811Server = createTexas811Server;
