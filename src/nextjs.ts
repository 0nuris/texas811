// Texas 811 Next.js route handler factory (optional entry point)
...
import { getTexas811TicketEligibility } from "./utils";
import type {
...
export interface Texas811RoutesConfig {
...
const itemAttributeValueSchema = z
  .object({
...
function isUpstreamTexas811Error(error: unknown): boolean {
  const message = getErrorMessage(error).toLowerCase();
  return (
    message.includes("texas 811") ||
    message.includes("ticket creation failed") ||
...
export function createTexas811Routes(config: Texas811RoutesConfig) {
      ...
      async GET(request: NextRequest) {
        ...
        try {
          ...
          const data = await server.searchTickets(token, {
            ...
            excavatorName:
              searchParams.get("excavatorName") || undefined,
          });
    ...
    "tickets/create": {
      async POST(request: Request) {
        ...
        try {
          ...
          const { ticketData: validatedTicketData, temporaryTicketId } =
            parsed.data;

          // Strip and override takenBy/callerEmail server-side
          const ticketData: Texas811CreateTicketData = {
            ...validatedTicketData,
            takenBy: username,
            excavator: {
        ...
        try {
          ...
          if (!updates.cancel) {
            ...
            if (!eligibility.canUpdate) {
              const error = eligibility.createdAt
                ? "Ticket cannot be updated until 72 hours after creation"
                : "Ticket eligibility unavailable: ticket creation date is missing or invalid";
          ...
          const status =
            message.includes("72 hours after creation") ||
            message.includes("eligibility unavailable")
              ? 409
              : 500;
        ...
        try {
          ...
          if (
            ...
            return NextResponse.json(
              {
                error:
                  "vendors array is required and must not be empty",
              },
              { status: 400 }
            );
          }
          ...
          if (!eligibility.canNoResponse) {
            const error = eligibility.createdAt
              ? "Ticket cannot submit no response until 72 hours after creation"
              : "Ticket eligibility unavailable: ticket creation date is missing or invalid";
...
export type Texas811Routes = ReturnType<typeof createTexas811Routes>;
